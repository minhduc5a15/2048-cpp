...
void GuiGame::draw() {
    ...
    if (isGameOver_) {
        ...
        DrawText(text, (GetScreenWidth() - textWidth)/2, GetScreenHeight()/2 - 50, fontSize, DARKGRAY);

        const char* subText = "Press ENTER to Restart";
        int subFontSize = 20;
        int subTextWidth = MeasureText(subText, subFontSize);
        DrawText(subText, (GetScreenWidth() - subTextWidth)/2, GetScreenHeight()/2 + 20, subFontSize, GRAY);
    }
